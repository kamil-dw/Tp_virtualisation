...
pipeline {
    ...
    environment {
        DOCKER_USERNAME = "kamildiaw" // username docker
    ...
    stages {
        ...
        stage("Checkout") {
            steps {
                git branch: 'master', url: 'https://github.com/kamil-dw/Tp_virtualisation.git'
            }
        }
        ...
        stage("Push image to Docker Hub") {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'TP_VIRTUALISATION', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASSWORD')]){
                    bat """
